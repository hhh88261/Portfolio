...
1. 사용자 로그인
2. 송신서버에 요청 전송
3. 송신서버에서 원시 AIS 메시지를 생성하여 수신서버로 전송  
4. 파싱 서버에서 TCP 소켓을 사용해 데이터를 수신 
...
### TCP vs Java NIO
가설 :   
현재 디코딩 서버의 데이터 수신 방식은 Polling 방식이다.즉, While 무한 루프를 돌며 데이터를 수신을 감시하고 있다는 것이다.   
서버가 지속적으로 작동할수록 처리량이 감소하고 CPU 사용률이 높아지는 문제가 발생했다.    
While 수신 방식을 이벤트 기반의 통신방식을 사용하면 이벤트가 발생할 때에만 CPU를 사용하므로 이러한 문제점을 개선할 수 있을 것이다.  
...
기존 블로킹 방식은 여러 스레드를 생성하여 데이터가 도착할때 까지 블로킹 상태로 대기하지만,    
NIO에서는 하나의 스레드만 이벤트를 감시하는 구조적 차이 때문인 것으로 보인다.  

고려사항 :  
수신량이 제한된 현재 프로젝트 구조에서는 기존 방식을 유지하는 것이 유리하다.    
다만 실제 AIS 송신 서버와 연결하여 수천개의 데이터를 수신해야 한다면 Java NIO 방식을 채용하는 것이 훨씬 우수할 것이다.  
